...
class AddStaffFrame(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        header = ctk.CTkLabel(header_frame, text="Add Staff Member", font=("Roboto Condensed", 24), text_color="black")
        ...
        hire_date_frame = ctk.CTkFrame(form_frame, fg_color="transparent")
        hire_date_frame.grid(row=8, column=1, sticky="w", padx=(0, 20), pady=10)

        entry = ctk.CTkEntry(hire_date_frame, width=150, height=self.ENTRY_HEIGHT,
                             border_width=self.BORDER_WIDTH, border_color=self.BORDER_COLOR,
                             placeholder_text="YYYY-MM-DD")
        entry.grid(row=0, column=0, sticky="w", padx=(0, 10))
        self.entries["entry_hire_date"] = entry

        today_button = ctk.CTkButton(hire_date_frame, text="Today", width=80, height=self.ENTRY_HEIGHT,
                                     command=self.set_today_date)
        today_button.grid(row=0, column=1, sticky="w")
    ...
    def set_today_date(self):
        ...
        self.entries["entry_hire_date"].delete(0, 'end')
        self.entries["entry_hire_date"].insert(0, today)
    ...
    def on_submit(self):
        ...
        try:
            employee_id = self.employee_model.add_employee(employee_data)
            messagebox.showinfo("Success", f"Staff member added successfully!\nEmployee ID: {employee_id}")
            if self.parent_page:
                self.parent_page.populate_employee_data()
            self.master.destroy()
        except Exception as e:
            log(f"Error adding staff member: {str(e)}")
    ...
    def reset_form(self):
        # Clear all entry fields and reset dropdowns
        for entry in self.entries.values():
            if isinstance(entry, ctk.CTkEntry):
                entry.delete(0, 'end')

        # Reset dropdowns to their first value
        self.gender_dropdown.set("")
        self.position_dropdown.set("")
        self.status_dropdown.set("Active")
